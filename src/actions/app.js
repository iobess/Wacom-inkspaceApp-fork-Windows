...
function setTheme(theme) {
	...
	return (dispatch, getState) => {
		dispatch({ type: ActionTypes.SET_THEME, body: theme });
		DBManager.edit(DBManager.entities.SETTINGS, { theme: theme }).catch(console.error);
	}
}

function setCanvasDarkMode(enabled) {
	return (dispatch, getState) => {
		dispatch({ type: ActionTypes.SET_CANVAS_DARK_MODE, body: enabled });
		DBManager.edit(DBManager.entities.SETTINGS, { canvasDarkMode: enabled }).catch(console.error);
	}
}
