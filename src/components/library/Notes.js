...
import { openWizard, openDialog } from '../../actions/modals';
		...
		return (
			<div>
				<div className="menu pull-left">
					<ul>
						<li>
							<Tooltip placement={'bottom'} destroyTooltipOnHide={true} overlay={<div ><FormattedMessage id={'tooltip.exportAll'} defaultMessage="Export All" /></div>} align={{ offset: [0, 10] }}>
								<a className="icon" onClick={() => window.contextMenuManager && window.contextMenuManager.showExportMenu()}>
									<ExportIcon />
								</a>
							</Tooltip>
					</li>
				...
				<li>
					<Tooltip placement={'bottom'} destroyTooltipOnHide={true} overlay={<div >Switch Connection</div>} align={{ offset: [0, 10] }}>
						<a className="icon" onClick={() => this.props.openDialog(Modals.SWITCH_CONNECTION_MODE)}>
							<LayerOptionsIcon />
						</a>
					</Tooltip>
				</li>
...
function mapDispatchToProps(dispatch) {
	return bindActionCreators({ ...actions, openWizard, openDialog }, dispatch);
}
